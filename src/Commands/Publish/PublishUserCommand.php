...
class PublishUserCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'vl-admin-tool.publish:user';
    ...
    public function handle()
    {
        ...
        if (config('admin_generator.laravel_generator.options.repository_pattern')) {
    ...
    {
        $viewsPath = config('vl_admin_tool.laravel_generator.path.views', resource_path('views/'));
        $templateType = config('vl_admin_tool.laravel_generator.templates', 'adminlte-templates');
    ...
    {
        $path = config('admin_generator.laravel_generator.path.routes', base_path('routes/web.php'));

        $routeContents = file_get_contents($path);

        $routesTemplate = get_template('routes.user', 'vl-admin-tool');
    ...
    {
        $templateData = get_template('user/user_controller', 'vl-admin-tool');
        if (!config('admin_generator.laravel_generator.options.repository_pattern')) {
            $templateData = get_template('user/user_controller_without_repository', 'vl-admin-tool');
            $templateData = $this->fillTemplate($templateData);
        }

        $templateData = $this->fillTemplate($templateData);

        $controllerPath = config('admin_generator.laravel_generator.path.controller', app_path('Http/Controllers/'));
    ...
    {
        $templateData = get_template('user/user_repository', 'vl-admin-tool');

        $templateData = $this->fillTemplate($templateData);

        $repositoryPath = config('admin_generator.laravel_generator.path.repository', app_path('Repositories/'));
    ...
    {
        $templateData = get_template('user/create_user_request', 'vl-admin-tool');

        $templateData = $this->fillTemplate($templateData);

        $requestPath = config('admin_generator.laravel_generator.path.request', app_path('Http/Requests/'));
    ...
    {
        $templateData = get_template('user/update_user_request', 'vl-admin-tool');
    ...
    {
        $templateData = str_replace('$NAMESPACE_CONTROLLER$', config('admin_generator.laravel_generator.namespace.controller'), $templateData);

        $templateData = str_replace('$NAMESPACE_REQUEST$', config('admin_generator.laravel_generator.namespace.request'), $templateData);

        $templateData = str_replace('$NAMESPACE_REPOSITORY$', config('admin_generator.laravel_generator.namespace.repository'), $templateData);
